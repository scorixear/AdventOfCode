    ...
    for ant in antennas.values():
        for i in range(0, len(ant)-1):
            for j in range(i+1, len(ant)):
                new_antinodes = find_antinodes(ant[i], ant[j])
                for antinode in new_antinodes:
                    if 0 <= antinode[0] < width and 0 <= antinode[1] < height:
                        found.add(antinode)
    print(len(found))

def find_antinodes(ant1: tuple[int, int], ant2: tuple[int, int]) -> list[tuple[int, int]]:
    dist = (ant1[0] - ant2[0], ant1[1] - ant2[1])
    return [
        (ant1[0] + dist[0], ant1[1] + dist[1]),
        (ant2[0] - dist[0], ant2[1] - dist[1])
    ]
