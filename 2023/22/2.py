...
class Node:
    ...
    def chain_remove(self, falling: set["Node"]):
        # number of bricks that fall when this brick is removed
        count = 0
        # parents that fall when this brick is removed
        to_fall: list[Node] = []
        # for each parent
        for parent in self.parents:
            ...
            if parent in falling:
                continue
            # count number of children that support this parent
            child_count = 0
            for child in parent.children:
                # if child is not falling, child supports parent
                if child not in falling:
                    child_count += 1
            # if no children support this parent
            if child_count == 0:
                # increase count for this parent
                count += 1
                # add this parent to falling
                falling.add(parent)
                # and recursively call chain_remove on this parent
                to_fall.append(parent)
        # for each parent that falls
        for node in to_fall:
            # recursively call chain_remove on this parent
            count += node.chain_remove(falling)
        return count
...
def main():
    ...
    for line in lines:
        ...
        # coordinates are inclusive
        # besdt coordinate is diagonal corners
    ...
    count = 0
    for brick in bricks:
        # chain remove this brick, and add count
        count += brick.chain_remove(set([brick]))
    print(count)
